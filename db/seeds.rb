...
# Create investment records for the admin user
admin_user.investments.find_or_create_by!(month: 0) do |investment|
...
(1..36).each do |month|
  admin_user.investments.find_or_create_by!(month: month) do |investment|
    investment.date = Date.new(2025, 6).advance(months: month - 1).strftime("%b %Y")
